            ...
            {contactInfo.map((info, idx) => (
              <div
                ...
                className={`transition-all duration-700 ${
                  isVisible[`info-${idx}`] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
                }`}
          ...
          <div className="grid lg:grid-cols-5 gap-8 md:gap-12 items-start">
            ...
            <div
              ...
              className={`lg:col-span-2 transition-all duration-1000 ${
                isVisible['left-content'] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'
              }`}
            >
                  ...
                  {features.map((feature, idx) => (
                    <div key={idx} className="bg-card backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-5 border border-border hover:border-primary/30 transition-all duration-300 hover:shadow-lg">
                      <div className={`w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br ${idx % 2 === 0 ? 'from-primary/10 to-primary/5' : 'from-secondary/10 to-secondary/5'} rounded-xl flex items-center justify-center mb-3`}>
                        <div className={idx % 2 === 0 ? 'text-primary' : 'text-secondary'}>{feature.icon}</div>
                      </div>
                      ...
                      {testimonials.map((testimonial, idx) => (
                        <div
                          ...
                          className={`absolute inset-0 transition-all duration-700 ${
                            activeTestimonial === idx ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'
                          }`}
                        >
                      ...
                      {testimonials.map((_, idx) => (
                        <button
                          ...
                          className={`h-2 rounded-full transition-all duration-300 ${
                            activeTestimonial === idx ? 'w-8 bg-primary' : 'w-2 bg-muted'
                          }`}
                        />
                      ))}
                    </div>
                      ...
                      return (
                        <button
                          ...
                          className={`w-10 h-10 md:w-12 md:h-12 bg-card border-2 ${
                            social.color === 'primary' ? 'border-primary/20 hover:bg-primary hover:border-primary' : 'border-secondary/20 hover:bg-secondary hover:border-secondary'
            ...
            <div
              ...
              className={`lg:col-span-3 transition-all duration-1000 delay-200 ${
                isVisible['right-content'] ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'
              }`}
            >
              {!isMessageOpen ? (
                <Card className="border-0 shadow-2xl bg-card/80 backdrop-blur-2xl overflow-hidden group hover:shadow-3xl transition-all duration-500 hover:-translate-y-2">
                  <CardContent className="p-8 md:p-12 lg:p-16">
                    <div className="text-center">
                      ...
                      <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-3 md:mb-4">Send Us a Message</h3>
                    ...
                    {isSubmitted ? (
                      ...
                      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 md:space-y-6">
                        <div className="grid md:grid-cols-2 gap-5 md:gap-6">
                          <div>
                            <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">Your Name</label>
                            <Input
                              {...register('name')}
                            ...
                            />
                            {errors.name && <p className="text-destructive text-xs md:text-sm mt-2">{errors.name.message}</p>}
                          </div>
                          <div>
                            <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">Email Address</label>
                            <Input
                              type="email"
                              {...register('email')}
                              className="w-full px-4 md:px-5 py-3 md:py-4 bg-background border-2 border-input rounded-xl md:rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all text-foreground placeholder:text-muted-foreground text-sm md:text-base"
                            ...
                            />
                            {errors.email && <p className="text-destructive text-xs md:text-sm mt-2">{errors.email.message}</p>}
                          </div>
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">Subject</label>
                          <Input
                            {...register('subject')}
                            className="w-full px-4 md:px-5 py-3 md:py-4 bg-background border-2 border-input rounded-xl md:rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all text-foreground placeholder:text-muted-foreground text-sm md:text-base"
                            placeholder="Tour inquiry or general question"
                          />
                          {errors.subject && <p className="text-destructive text-xs md:text-sm mt-2">{errors.subject.message}</p>}
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">Your Message</label>
                          <Textarea
                            {...register('message')}
                            rows={5}
                          ...
                          />
                          {errors.message && <p className="text-destructive text-xs md:text-sm mt-2">{errors.message.message}</p>}
                        </div>
                        <Button
                          type="submit"
      ...
      <section className="py-20 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-muted-foreground">Everything you need to know about TravelMate</p>
          </div>
