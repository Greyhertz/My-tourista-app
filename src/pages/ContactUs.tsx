...
import { Send, Mail, Phone, MapPin, Clock, MessageCircle, X, Check, Facebook, Twitter, Instagram, Linkedin, Globe, Plane, Users, Award, Camera, Mountain, CheckCircle } from 'lucide-react';
...
import { type SignUpFormData, signUpSchema } from '@/utils/validateForm';
...
const ContactPage = () => {
  ...
  export default function ContactUs() {
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              setIsVisible((prev) => ({
                ...prev,
                [(entry.target as HTMLElement).dataset.observeId as string]: true
              }));
            }
          });
        },
    ...
    const setObserverRef = (el: HTMLElement | null, id: string) => {
    ...
    return (
      <div className="bg-background text-foreground font-sm">
        ...
        <section className="relative pt-16 pb-12 md:pt-24 lg:pt-32 md:pb-16 lg:pb-20 px-4 md:px-6 lg:px-8 overflow-hidden">
          ...
          <div
            ref={(el) => setObserverRef(el, 'hero')}
            className={`max-w-5xl mx-auto text-center relative z-10 transition-all duration-1000 ${isVisible['hero'] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
              }`}
          >
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-primary/10 to-secondary/10 px-4 md:px-6 py-2 md:py-3 rounded-full mb-6 md:mb-8 backdrop-blur-sm border border-primary/20">
              <Plane className="w-4 h-4 md:w-5 md:h-5 text-primary" />
              <span className="text-primary font-semibold text-sm md:text-base">We'd Love to Hear From You</span>
            </div>
            <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold text-foreground mb-4 md:mb-8 leading-tight px-4">
              Get in <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Touch</span>
            </h1>
            <p className="text-base md:text-xl lg:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed px-4">
              Have questions about your next adventure? Our travel experts are here to help you plan the perfect journey across the globe.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {contactInfo.map((info, idx) => (
                <div
                  ...
                  className={`transition-all duration-700 ${isVisible[`info-${idx}`] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
                    }`}
                ...
                >
                  <Card className="h-full border-0 shadow-lg hover:shadow-2xl transition-all duration-500 group bg-card backdrop-blur-md hover:-translate-y-2">
                    <CardContent className="p-5 md:p-6 lg:p-8">
                      <div className={`w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-gradient-to-br ${info.color} rounded-2xl flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg`}>
                        <div className={info.iconColor}>
                          {info.icon}
                        </div>
                      </div>
                      <h3 className="text-base md:text-lg font-bold text-card-foreground mb-2">{info.title}</h3>
                      <p className="text-sm md:text-base text-foreground font-semibold mb-1">{info.details}</p>
                      <p className="text-xs md:text-sm text-muted-foreground">{info.subtext}</p>
                    </CardContent>
                  </Card>
                </div>
              ))}
          ...
          <div className="max-w-[1300px] mx-auto">
            <div className="grid lg:grid-cols-5 gap-8 md:gap-12 items-start">
              ...
              <div
                ...
                className={`lg:col-span-2 transition-all duration-1000 ${isVisible['left-content'] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'
                  }`}
              >
                <div className="space-y-6 md:space-y-8">
                  <div>
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4 md:mb-6 leading-tight">
                      Let's Start Planning Your <span className="text-primary">Adventure</span>
                    </h2>
                    <p className="text-base md:text-lg text-muted-foreground leading-relaxed">
                      Whether you're looking for a relaxing beach escape, an adrenaline-filled adventure, or a cultural immersion, we're here to make it happen.
                    </p>
                  </div>

                  <div className="grid grid-cols-2 gap-3 md:gap-4">
                    {features.map((feature, idx) => (
                      <div key={idx} className="bg-card backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-5 border border-border hover:border-primary/30 transition-all duration-300 hover:shadow-lg">
                        <div className={`w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br ${idx % 2 === 0 ? 'from-primary/10 to-primary/5' : 'from-secondary/10 to-secondary/5'} rounded-xl flex items-center justify-center mb-3`}>
                          <div className={idx % 2 === 0 ? 'text-primary' : 'text-secondary'}>
                            {feature.icon}
                          </div>
                        </div>
                        <h4 className="font-bold text-foreground mb-1 text-xs md:text-sm">{feature.title}</h4>
                  ...
                  <Card className="border-0 shadow-xl bg-card backdrop-blur-sm overflow-hidden">
                    <CardContent className="p-6 md:p-8">
                      <h3 className="text-base md:text-lg font-bold text-foreground mb-4 md:mb-6">What Travelers Say</h3>
                      <div className="relative h-40 md:h-48">
                        {testimonials.map((testimonial, idx) => (
                          <div
                            key={idx}
                            className={`absolute inset-0 transition-all duration-700 ${activeTestimonial === idx ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'
                              }`}
                          >
                            <div className="flex items-start gap-3 md:gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm md:text-base text-muted-foreground italic mb-3 md:mb-4 leading-relaxed">{testimonial.text}</p>
                                <div>
                      ...
                      <div className="flex justify-center gap-2 mt-4">
                        {testimonials.map((_, idx) => (
                          <button
                            ...
                            className={`h-2 rounded-full transition-all duration-300 ${activeTestimonial === idx ? 'w-8 bg-primary' : 'w-2 bg-muted'
                              }`}
                          />
                        ))}
                      </div>
                  ...
                  <div>
                    <h3 className="text-base md:text-lg font-bold text-foreground mb-4">Connect With Us</h3>
                    <div className="flex gap-3">
                      {[
                        ...
                        return (
                          <button
                            key={idx}
                            className={`w-10 h-10 md:w-12 md:h-12 bg-card border-2 ${social.color === 'primary' ? 'border-primary/20 hover:bg-primary hover:border-primary' : 'border-secondary/20 hover:bg-secondary hover:border-secondary'} rounded-xl flex items-center justify-center transition-all duration-300 group hover:scale-110 hover:shadow-lg`}
                          >
                            <Icon className={`w-5 h-5 ${social.color === 'primary' ? 'text-primary' : 'text-secondary'} group-hover:text-white transition-colors`} />
                          </button>
                        );
                      })}
                    </div>
              ...
              <div
                ...
                className={`lg:col-span-3 transition-all duration-1000 delay-200 ${isVisible['right-content'] ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'
                  }`}
              >
                {!isMessageOpen ? (
                    ...
                    <CardContent className="p-8 md:p-12 lg:p-16">
                      <div className="text-center">
                        ...
                        <p className="text-base md:text-lg text-muted-foreground mb-6 md:mb-8 max-w-md mx-auto px-4">
                          Click below to open our contact form and share your travel dreams with us
                        </p>
                        <Button
                  ...
                  <Card className="border-0 shadow-2xl bg-card/90 backdrop-blur-2xl overflow-hidden">
                    <CardContent className="p-6 md:p-8 lg:p-10">
                      <div className="flex items-center justify-between mb-6 md:mb-8">
                        <h3 className="text-2xl md:text-3xl font-bold text-foreground">Contact Form</h3>
                        <Button
                          variant="ghost"
                          size="icon"
                      ...
                      {isSubmitted ? (
                        <div className="text-center py-12 md:py-16">
                          ...
                          <h4 className="text-xl md:text-2xl font-bold text-foreground mb-3">Message Sent Successfully!</h4>
                          <p className="text-muted-foreground text-base md:text-lg">We'll get back to you within 24 hours.</p>
                        </div>
                      ) : (
                        <div className="space-y-5 md:space-y-6">
                          <div className="grid md:grid-cols-2 gap-5 md:gap-6">
                            <div>
                              ...
                              />
                              {errors.name && (
                                <p className="text-destructive text-xs md:text-sm mt-2">{errors.name.message}</p>
                              )}
                            </div>
                            <div>
                              <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">
                              ...
                              />
                              {errors.email && (
                                <p className="text-destructive text-xs md:text-sm mt-2">{errors.email.message}</p>
                              )}
                            </div>
                          </div>
                          <div>
                            ...
                            />
                            {errors.subject && (
                              <p className="text-destructive text-xs md:text-sm mt-2">{errors.subject.message}</p>
                            )}
                          </div>
                          <div>
                            <label className="block text-sm font-bold text-foreground mb-2 md:mb-3">
                            ...
                            />
                            {errors.message && (
                              <p className="text-destructive text-xs md:text-sm mt-2">{errors.message.message}</p>
                            )}
                          </div>
                          <Button
              ...
              <div className="relative h-full flex items-center">
                <div className="px-6 md:px-12 lg:px-16 max-w-2xl">
                  <div className="inline-flex items-center gap-2 md:gap-3 bg-white/10 backdrop-blur-md px-4 md:px-6 py-2 md:py-3 rounded-full mb-4 md:mb-6 border border-white/20">
                    ...
                    <span className="text-white font-semibold text-sm md:text-base">Find Us</span>
                  </div>
                  <h3 className="text-3xl md:text-5xl font-bold text-white mb-4 md:mb-6 leading-tight">Visit Our Office</h3>
                  <p className="text-base md:text-xl text-white/90 mb-6 md:mb-8 leading-relaxed">
                    Schedule an appointment to meet with our travel experts in person and start planning your next adventure.
                  </p>
      ...
      <style jsx>{`
        ...
        <section className="py-16 md:py-24 px-4 md:px-6 lg:px-8 bg-gradient-to-br from-primary/10 via-background to-secondary/10">
          <div
            ref={(el) => setObserverRef(el, 'final-cta')}
            className={`max-w-4xl mx-auto text-center transition-all duration-1000 ${isVisible['final-cta'] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
              }`}
          >
            <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4 md:mb-6">
              Ready to Explore the World?
